...
intapps_running()
	{
		; variables
		url1 := "https://intapps.carnet.hr/" ; url opened in new chrom instance
		; js intapps
		 intaps_ime = document.querySelector("#username").value =%user%  ; username
  ...
  if PageInst {
	...
	if (PageInst) {
		...
		PageInst.Evaluate(intaps_ime)
	  PageInst.Evaluate(intaps_loz)        ; execute JS
	  try {
	    PageInst.Evaluate(intaps_prijava)
	  }
	  catch e
	  {
	    msgBox, Wrong username or password !! Exiting...
	    ExitApp
	  }
		PageInst.Evaluate(raspored)
	  msgBox, raspored opened
	  return
	 }
	 else
		new_intapps()
...
new_intapps()
{
		...
		; variables
		url:= "https://intapps.carnet.hr/hdraspored/public/schedule/index" ; url opened in new chrom instance
		; js intapps
		 intaps_ime = document.querySelector("#username").value = "user";username
		 intaps_loz = document.querySelector("#password").value ="pass" ; password
		 intaps_prijava = document.querySelector("body > div.container.login-container > div > div > form > div.form-group.row.justify-content-md-center > div > button").click()     ; click submit button defined by js path
...
crm_running()				; check if CRM running
  ...
  if PageInst {
	...
	/*
	  ...
	  try {
	  PageInst.Evaluate(crm_ime)
	  }
	  try {
	    PageInst.Evaluate(crm_prijava)
	  }
	  catch e
	  {
	    msgBox, Wrong username or password !! Exiting...
...
class Chrome
{
	...
	*/
	__New(ProfilePath:="", URLs:="", Flags:="", ChromePath:="", DebugPort:="")
	{
		; Verify ProfilePath
